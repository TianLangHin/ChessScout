...
struct ChessboardView: View {
    ...
    var body: some View {
        VStack {
            ...
            TextField("Enter move here:", text: $moveSan)
                .textInputAutocapitalization(.never)
                .autocorrectionDisabled()
                .padding()

            Button {
                if let move = Move(san: moveSan, position: board.model.board.position) {
                    withAnimation(.linear(duration: 0.25)) {
                        let _ = board.makeMove(move: move)
                    }
                }
            } label: {
                Text("Enter Move!")
    ...
    func boardLayer(squareSize: CGFloat) -> some View {
        VStack(spacing: 0) {
